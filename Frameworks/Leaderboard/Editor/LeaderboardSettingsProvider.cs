...
namespace Com.Hapiga.Scheherazade.Common.Leaderboard.Editor
{
    internal sealed class LeaderboardSettingsProvider : SettingsProvider
    {
        ...
        private const string PlatformTabPrefKey =
            "LeaderboardSettingsProvider_PlatformTab";
        ...
        public static SettingsProvider CreateSettingsProvider()
        {
            return new LeaderboardSettingsProvider(
                "Project/Frameworks/Leaderboards",
        ...
        private void DrawStatusBar(LeaderboardConfiguration config)
        {
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                int providerCount = (hasAndroid ? 1 : 0) + (hasIos ? 1 : 0);
                int leaderboardCount = config.Leaderboards.Length;
                bool valid = config.HasAnyProvider && leaderboardCount > 0;
                ...
                if (hasAndroid) platforms += "Android ";
                if (hasIos) platforms += "iOS";
                if (string.IsNullOrEmpty(platforms)) platforms = "None";

                EditorGUILayout.LabelField(
                    $"Status: {statusText}  |  "
                    + $"{providerCount} provider(s) [{platforms}], "
                    + $"{leaderboardCount} leaderboard(s)",
                    style
                );
            }
        }
        ...
        private void DrawManageTab()
        {
            ...
            // Platform tabs switch which provider field we edit
        ...
        {
            var settings = LeaderboardConfiguration.Instance;
            ...
            if (GUILayout.Button(
                    ...
                    GUILayout.Height(30)))
            {
                LeaderboardConfiguration.CreateOrMoveToDesignatedPath();
            }
