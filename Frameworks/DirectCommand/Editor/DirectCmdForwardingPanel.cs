...
namespace Com.Hapiga.Scheherazade.Editor
{
    internal sealed partial class DirectCmdSettingsProvider
    {
        ...
        private Vector2 _detectedAppsScrollPosition;
        private bool _showHistoryPanel;
        private bool _showDetectedApps;
        private bool _commandForwardingInitialized;
        private Vector2 _commandForwardingScrollPosition;
        ...
        private bool HasSelectedDevice => _selectedDeviceIndex >= 0
            && _selectedDeviceIndex < _devices.Count;
        #endregion

        #region Unity Callbacks
        private void InitializeCommandForwarding()
        {
            if (_commandForwardingInitialized)
            {
                return;
            }
            ...
            ResolveAdbPath();
            RefreshDevicesAsync();
            _commandForwardingInitialized = true;
        }

        private void PersistCommandForwarding()
        {
            if (!_commandForwardingInitialized)
            {
                return;
        ...
        private void DrawCommandForwardingTab()
        {
            _commandForwardingScrollPosition = EditorGUILayout.BeginScrollView(
                _commandForwardingScrollPosition
            );
            DrawTargetSelector();
        ...
        private void DrawTargetSelector()
        {
            ...
            {
                EditorGUILayout.HelpBox(
                    "Select an Android device and provide a package ID to enable command forwarding.",
                    MessageType.Info
                );
            }
...
private void DrawDeviceSelector()
        {
            ...
            bool canExport = !string.IsNullOrEmpty(_packageId) && HasSelectedDevice;
        ...
        private void DrawPackageIdRow()
        {
            ...
            DrawDetectedPackageSelector();
        }

        private void DrawDetectedPackageSelector()
        {
            if (!_showDetectedApps || _detectedApps.Count == 0)
            {
                return;
            }
            ...
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                {
                    EditorGUILayout.LabelField(
                        $"Running Packages ({_detectedApps.Count})",
                ...
                EditorGUILayout.LabelField(
                    "Type in Package ID to filter, then select a running package.",
                    EditorGuiStyles.InlineStatus
                );

                int matchCount = 0;
                _detectedAppsScrollPosition = EditorGUILayout.BeginScrollView(
                    _detectedAppsScrollPosition,
                    GUILayout.MaxHeight(150f)
                );

                for (int i = 0; i < _detectedApps.Count; i++)
                {
                    DetectedAppInfo app = _detectedApps[i];
                    if (!MatchesPackageSearch(app.packageId, _packageId))
                    {
                        continue;
                    }
                    ...
                    matchCount++;
                    if (GUILayout.Button(
                            new GUIContent(app.packageId, app.dataPath),
                            EditorGuiStyles.ModeButtonInactive
                        ))
                    {
                        SelectDetectedApp(app);
                    }
                }
                ...
                {
                    EditorGUILayout.LabelField(
                        "No running package matches the current search.",
                        EditorGuiStyles.InlineStatus
                    );
                }
            }
        ...
        {
            if (string.IsNullOrEmpty(_packageId) || !HasSelectedDevice)
                return;
        ...
        private async Task<bool> SendToDeviceAsync(string commands)
        {
            ...
            {
                _statusMessage = "Select a connected Android device before sending commands.";
                _statusType = MessageType.Warning;
                return false;
            }
        ...
        private async void DetectAppsAsync()
        {
            ...
            try
            {
                ...
                await DetectRunningAppsAsync(device.Serial, externalStorage);
                _detectionState = AppDetectionState.Detected;
                _showDetectedApps = _detectedApps.Count > 0;

                if (TrySelectMatchingDetectedApp())
                {
                    _statusMessage = $"Selected running package '{_packageId}'.";
                }
                else if (_detectedApps.Count > 0)
                {
                    _statusMessage =
                        $"Detected {_detectedApps.Count} running package(s). Select one below.";
                }
                else
                {
                    _statusMessage = "No running application packages were detected on this device.";
                    _statusType = MessageType.Warning;
                }
        ...
        private async Task DetectRunningAppsAsync(string serial, string externalStorage)
        {
            AdbResult result = await RunAdbAsync(serial, "shell \"ps -A -o NAME\"");
            if (result.ExitCode != 0)
            {
                result = await RunAdbAsync(serial, "shell ps");
            }
            ...
            if (result.ExitCode != 0)
            {
                throw new InvalidOperationException(
                    string.IsNullOrWhiteSpace(result.StandardError)
                        ? "Unable to list Android processes."
                        : result.StandardError.Trim()
                );
            }

            HashSet<string> packageIds = ParseRunningPackageIds(result.StandardOutput);
            var sortedPackageIds = new List<string>(packageIds);
            sortedPackageIds.Sort(StringComparer.OrdinalIgnoreCase);

            foreach (string packageId in sortedPackageIds)
            {
                string dataPath = BuildAppDataPath(externalStorage, packageId);
                _detectedApps.Add(new DetectedAppInfo
                {
                    packageId = packageId,
                    dataPath = dataPath,
                    dataPathExists = false,
                    displayLabel = $"{packageId}  \u279C  {dataPath}"
                });
            }
        }

        private bool TrySelectMatchingDetectedApp()
        {
            if (_detectedApps.Count == 1)
            {
                SelectDetectedApp(_detectedApps[0]);
                return true;
            }
            ...
            {
                DetectedAppInfo app = _detectedApps[i];
                if (string.Equals(app.packageId, _packageId, StringComparison.OrdinalIgnoreCase))
                {
                    SelectDetectedApp(app);
                    return true;
                }
        ...
        private void SelectDetectedApp(DetectedAppInfo app)
        {
            _packageId = app.packageId;
            _showDetectedApps = false;
            GUI.FocusControl(null);
        }

        private static HashSet<string> ParseRunningPackageIds(string output)
        {
            ...
            for (int i = 0; i < lines.Length && packageIds.Count < MaxDetectedApps; i++)
            {
                string candidate = ExtractProcessName(lines[i]);
                string packageId = NormalizePackageId(candidate);
                if (IsAndroidPackageId(packageId))
                {
        ...
        private static string ExtractProcessName(string processLine)
        {
            string trimmed = processLine.Trim();
            if (string.Equals(trimmed, "NAME", StringComparison.OrdinalIgnoreCase))
            {
                return string.Empty;
            }

            int separatorIndex = trimmed.LastIndexOfAny(new[] { ' ', '\t' });
            return separatorIndex >= 0
                ? trimmed.Substring(separatorIndex + 1)
                : trimmed;
        }

        private static string NormalizePackageId(string processName)
        {
            int separatorIndex = processName.IndexOf(':');
            return separatorIndex >= 0
                ? processName.Substring(0, separatorIndex)
                : processName;
        }

        private static bool IsAndroidPackageId(string packageId)
        {
            if (string.IsNullOrWhiteSpace(packageId) || packageId.IndexOf('.') <= 0)
            {
                return false;
            }
            ...
            for (int i = 0; i < segments.Length; i++)
            {
                ...
                for (int charIndex = 1; charIndex < segment.Length; charIndex++)
                {
                    char character = segment[charIndex];
        ...
        private static bool MatchesPackageSearch(string packageId, string searchText)
        {
            return string.IsNullOrWhiteSpace(searchText)
                || packageId.IndexOf(searchText.Trim(), StringComparison.OrdinalIgnoreCase) >= 0;
        }
        ...
        {
            AdbPathResolver.InvalidateCache();
            _adbPath = AdbPathResolver.AdbPath;
        }
